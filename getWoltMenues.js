...
const filterKeyWordsFile = fs.readFileSync("filterKeyWords.json");
...
function getRestaurantsData() {
  const restaurantsFile = fs.readFileSync("woltRestaurantsData.json");
...
async function main() {
  ...
  fs.writeFileSync("woltMenuesData.json", jsonData);
  //   await Promise.all(
  //     urls.map(
  //       async (url) => await getData(url).then((res) => data.push(res.data))
  //     )
  //   );
